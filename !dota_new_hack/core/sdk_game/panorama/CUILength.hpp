...
#define RoundFloatToInt(fl) std::roundf(fl)
	...
	void ScaleLengthValue( float flScaleFactor )
	{
		if ( m_eType == k_EUILengthLength && m_flValue != k_flFloatAuto && m_flValue != k_flFloatNotSet )
		{
			...
			else
			{
				...
				if ( fabsf( m_flValue - RoundFloatToInt( m_flValue ) ) < 0.001f )
					m_flValue = RoundFloatToInt( m_flValue * flScaleFactor );
				else
...
public:
	...
	CStyleSymbol( uint8_t id ) : m_Id( id ) {}
	...
	// Is valid?
	bool is_valid( ) const { return m_Id != 0xFF; }
