...
public:
    ...
    CFileHandle* OpenFile( const char* file_path, const char* options, int unk = 0, const char* path_id = nullptr ) {
        return CallVFunc<78, CFileHandle*>( file_path, options, unk, path_id );
    }

    void Close( CFileHandle* handle ) {
        return CallVFunc<14, void>( handle );
    }

    unsigned int GetFileSize( CFileHandle* handle ) {
        return CallVFunc<18, unsigned int>( handle );
    }

    unsigned int Write( void const* input, int size, CFileHandle* file ) {
    ...
    int ReadEx( char* buffer, int size_dest, int size, CFileHandle* handle ) {
