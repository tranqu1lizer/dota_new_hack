...
public:
	...
	T& Element(I i) { return m_Elements[i].m_Data; }
	T const& Element(I i) const { return m_Elements[i].m_Data; }
	T& operator[](I i) { return m_Elements[i].m_Data; }
	T const& operator[](I i) const { return m_Elements[i].m_Data; }

	// Gets the root
	I  Root() const { return m_Root; }

	// Num elements
	unsigned int Count() const { return (unsigned int)m_NumElements; }
	...
	I  MaxElement() const { return (I)m_Elements.NumAllocated(); }

	// Gets the children                               
	I  Parent(I i) const { return Links(i).m_Parent; }
	I  LeftChild(I i) const { return Links(i).m_Left; }
	I  RightChild(I i) const { return Links(i).m_Right; }

	// Tests if a node is a left or right child
	bool  IsLeftChild(I i) const { return LeftChild(Parent(i)) == i; }
	bool  IsRightChild(I i) const { return RightChild(Parent(i)) == i; }

	// Tests if root or leaf
	bool  IsRoot(I i) const { return i == m_Root; }
	bool  IsLeaf(I i) const { return (LeftChild(i) == InvalidIndex()) && (RightChild(i) == InvalidIndex()); }
	...
	I  FirstPreorder() const { return m_Root; }
...
protected:
	...
	Links_t& Links(I i) { return *(Links_t*)&m_Elements[i]; }

	// Checks if a link is red or black
	bool IsRed(I i) const { return (Links(i).m_Tag == RED); }
	bool IsBlack(I i) const { return (Links(i).m_Tag == BLACK); }
	...
	M const& Elements() const { return m_Elements; }
};